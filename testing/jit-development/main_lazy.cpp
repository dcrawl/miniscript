...
// Phase 2.2: Lazy loading helpers
static bool shellIntrinsicsLoaded = false;
static bool termIntrinsicsLoaded = false;

static bool IsShellFunction(const String& funcName) {
	static const char* SHELL_FUNCTIONS[] = {
		"exit", "shellArgs", "env", "input", "import", "file", "_dateVal", "_dateStr",
		"exec", "RawData", "key", "version", "print", "clear", "reset", "stackTrace", "debugMode"
	};
	static const int numFunctions = sizeof(SHELL_FUNCTIONS) / sizeof(SHELL_FUNCTIONS[0]);
	
	for (int i = 0; i < numFunctions; i++) {
		if (funcName == SHELL_FUNCTIONS[i]) {
			return true;
		}
	}
	return false;
...
static bool IsTermFunction(const String& funcName) {
	static const char* TERM_FUNCTIONS[] = {
		"term_raw", "term_read", "term_size", "term"
	};
	static const int numFunctions = sizeof(TERM_FUNCTIONS) / sizeof(TERM_FUNCTIONS[0]);
	
	for (int i = 0; i < numFunctions; i++) {
		if (funcName == TERM_FUNCTIONS[i]) {
			return true;
		}
	}
	return false;
...
static void PreloadRequiredIntrinsics(const String& code) {
	// Scan the code for function names that require shell or terminal intrinsics
	if (!shellIntrinsicsLoaded) {
		const char* SHELL_FUNCTIONS[] = {
			"exit", "shellArgs", "env", "input", "import", "file", "_dateVal", "_dateStr",
			"exec", "RawData", "key", "version", "print", "clear", "reset", "stackTrace", "debugMode"
		};
		const int numShellFuncs = sizeof(SHELL_FUNCTIONS) / sizeof(SHELL_FUNCTIONS[0]);
	...
	if (!termIntrinsicsLoaded) {
		const char* TERM_FUNCTIONS[] = {
			"term_raw", "term_read", "term_size", "term"
		};
		const int numTermFuncs = sizeof(TERM_FUNCTIONS) / sizeof(TERM_FUNCTIONS[0]);
		
		for (int i = 0; i < numTermFuncs; i++) {
			if (code.IndexOf(String(TERM_FUNCTIONS[i])) != -1) {
				AddTermIntrinsics();
				termIntrinsicsLoaded = true;
				break;
			}
		}
	}
...
static int DoCommand(Interpreter &interp, String cmd) {
	// Phase 2.2: Preload required intrinsics based on code analysis
	PreloadRequiredIntrinsics(cmd);
...
int main(int argc, const char * argv[]) {
	...
	AddPathEnvVars();
	AddScriptPathVar("");
	// Phase 2.2: Remove eager intrinsic loading - now done lazily on demand
	// AddShellIntrinsics();  // Now loaded lazily when first shell function is accessed
	// AddTermIntrinsics();   // Now loaded lazily when first terminal function is accessed
